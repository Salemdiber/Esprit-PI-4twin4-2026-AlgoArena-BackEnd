...
export class AIAnalysisService {
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are an AI coding assistant. Give short, concise, and helpful feedback.',
          },
  ...
  async generateHint(
    ...
    try {
      ...
      if (hintLevel === 1)
        instructions =
          'Give only a general direction for solving the challenge.';
      else if (hintLevel === 2)
        instructions =
          'Suggest a specific technique or data structure to solve it.';
      else
        instructions =
          'Explain a detailed step-by-step approach without writing the actual code.';
        ...
        messages: [
          {
            role: 'system',
            content:
              'You are a technical hint generator. Be concise and do not write complete code blocks.',
          },
          {
            role: 'user',
            content: `Challenge: ${challengeTitle}\nDescription: ${challengeDescription}\n\n${instructions}`,
    ...
    try {
      ...
      const safeTime =
        !parsedTime || parsedTime === 'Unknown'
          ...
          : parsedTime;
      const safeSpace =
        !parsedSpace || parsedSpace === 'Unknown'
          ? fallback.spaceComplexity
          : parsedSpace;
      const qualityScore = Number.isFinite(Number(parsed?.codeQualityScore))
        ? Math.max(
            0,
            Math.min(100, Math.round(Number(parsed.codeQualityScore))),
          )
        : fallback.codeQualityScore;
      const qualityNotes =
        Array.isArray(parsed?.codeQualityNotes) &&
        parsed.codeQualityNotes.length
      ...
      return {
        ...
        aiDetection:
          parsed?.aiDetection === 'AI_SUSPECTED' ? 'AI_SUSPECTED' : 'MANUAL',
        recommendations:
  ...
  private estimateSubmissionDetails(userCode: string): {
    ...
    const timeComplexity =
      loopCount >= 2 ? 'O(n^2)' : loopCount === 1 ? 'O(n)' : 'O(1)';
    ...
    return {
      ...
      aiDetection: 'MANUAL',
      recommendations: [
        'Consider documenting edge-case handling to improve clarity.',
      ],
      codeQualityScore: 65,
      codeQualityNotes: [
        'Improve naming and add brief comments for tricky parts.',
      ],
    };
  }
}
