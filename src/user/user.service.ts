...
export class UserService {
  ...
  async saveSpeedTestSession(userId: string, sessionData: any): Promise<void> {
    this.ensureValidObjectId(userId);
    // Validate languages: only allow 'python' or 'javascript' for speed challenges
    if (sessionData && sessionData.languages && typeof sessionData.languages === 'object') {
      const allowed = new Set(['python', 'javascript']);
      for (const key of Object.keys(sessionData.languages)) {
        const val = (sessionData.languages[key] || '').toString().toLowerCase();
        if (!allowed.has(val)) {
          throw new BadRequestException('Only python and javascript are allowed for speed challenges');
        }
        // normalize stored value to lowercase
        sessionData.languages[key] = val;
      }
    }
