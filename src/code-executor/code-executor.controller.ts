...
export class CodeExecutorController {
  ...
  async execute(@Body() body: any) {
    ...
    if (!code)
      throw new BadRequestException(this.tr('codeExecutor.codeRequired'));
    if (!language)
      throw new BadRequestException(this.tr('codeExecutor.languageRequired'));

    try {
      if (Array.isArray(testCases) && testCases.length > 0) {
        // Run validation against provided test cases
        const result = await this.executor.validateCode(
          code,
          language,
          testCases.map((t: any) => ({
